...
exports.groupCreation = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "unable to create a group" });
  }
};
...
exports.allMembersInGroup = async (req, res) => {
  try {
    ...
    console.log(groupId);
    const groupMembersId = await userGroup.findAll({
      where: { groupId: groupId },
      attributes: ["userId", "isAdmin"],
    });

    const members = await Promise.all(
      groupMembersId.map(async (data) => {
        const membersName = await User.findAll({
    ...
    return res
      .status(500)
      .json({ success: false, message: "unable to retrieve group members" });
  }
};
...
exports.addUserToGroup = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "failed at userCheck function" });
  }
};
...
async function userCheck(groupId, userId, isAdmin) {
  try {
    ...
    const userInGroupOrNot = await userGroup.findOne({
...
exports.makeAdmin = async (req, res) => {
  try {
    ...
    const userAdminOrNot = await userGroup.findOne({
      where: { groupId: groupId, userId: userId },
      attributes: ["isAdmin"],
    });
    ...
    if (userAdminOrNot.isAdmin === false) {
      return res
        .status(401)
        .json({ success: true, message: "user is not an admin" });
    }
