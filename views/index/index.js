...
async function getAll() {
  try {
    getAllClicked = true;
    const groupId = localStorage.getItem("groupId");
    const allMsgsRes = await axios.get(
      `http://localhost:3000/msg/toget/${groupId}`
    );
...
function groupUI(data) {
  ...
  data.forEach((item) => {
    ...
    groupText.addEventListener("click", () =>
      switchGroup(item.id, item.GroupName)
    );
...
async function addUserClicked(e) {
  ...
  try {
    ...
    const userCheckInGroupRes = await axios.get(
      `http://localhost:3000/group/toadduser?userEmail=${userName}&groupId=${groupId}&admin=false`
    );
...
//     const userCheckInGroupRes = await axios.get(
//       `http://localhost:3000/group/toadduser?groupId=${id}&admin=false`
//     );
//     console.log(userCheckInGroupRes.status);
//     if (userCheckInGroupRes.status === 200) {
...
async function allGroupMembers(id) {
  try {
    const allMembersRes = await axios.get(
      `http://localhost:3000/group/togetAllusers?groupId=${id}`
    );
...
async function adminButton(id) {
  try {
    ...
    const makeAdminRes = await axios.put(
      `http://localhost:3000/group/makeAdmin?userId=${id}&groupId=${groupId}`
    );
...
async function deleteButton(id) {
  try {
    ...
    const deleteRes = await axios.delete(
      `http://localhost:3000/group/deleteUser?userId=${id}&groupId=${groupId}`
    );
